...
namespace stateObservation
{
    namespace tools
    {
        ...
        namespace kinematics
        {
            ...
            {
                Matrix3 r2 = (rotationVectorToAngleAxis( - state.segment(9,3))).
                             toRotationMatrix();//inverse
                Vector3 omega1 = state.segment(12,3);
                Vector3 omega1dot = state.segment(15,3);
                Matrix3 omega1x = skewSymmetric(omega1);
                Matrix3 omega1dotx = skewSymmetric(omega1dot);

                Vector3 t1 = state.head(3);
                Vector3 t1dot = state.segment(3,3);
                Vector3 t1dotdot = state.segment(6,3);



                Vector state2(Vector::Zero(18,1));
                state2.head(3)= - r2 * t1 ;   //t2
                state2.segment(3,3)= r2 * ( omega1x * t1 - t1dot); //t2dot
                state2.segment(6,3)=  r2 * ( omega1x * (2 * t1dot - omega1x * t1)
                                             - t1dotdot + omega1dotx * t1); //t2dotdot
                state2.segment(9,3)= -state.segment(9,3);   //thetaU2
                state2.segment(12,3)= -r2 * omega1; //omega2
                state2.segment(15,3)=  r2 * (omega1x * omega1 - omega1dot); //omega2dot
